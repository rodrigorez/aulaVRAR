/**
 * 🌌 SHADER SHADERI - Convertido do Shadertoy
...
const shaderIFragmentShader = `uniform float time;
...
varying vec2 vUv;
    ...
    float pattern(vec2 uv, float time, inout vec2 q, inout vec2 r) {
      ...
      resolution = vec2( fbm1( uv * .1 + 4.0*q + vec2(1.7 - time / 2.,9.2) ),
                     fbm1( uv + 4.0*q + vec2(8.3 - time / 2.,2.8) ) );

      vec2 s = vec2( fbm1( uv + 5.0*resolution + vec2(21.7 - time / 2.,90.2) ),
                     fbm1( uv * .05 + 5.0*resolution + vec2(80.3 - time / 2.,20.8) ) ) * .25;
...
void main()
{
    ...
    vec2 uv = getScreenSpace();
      ...
      float time = (time * timeMultiplier) / 10.;
    ...
    _pattern = pattern(uv, time, q, r);
      ...
      colour.resolution -= dot(q, r) * 15.;
      colour = mix(colour, vec3(pattern(resolution, time, q, r), dot(q, r) * 15., -0.1), .5);
      colour -= q.y * 1.5;
...
// 🚀 Registra o shader
registrarShader('shaderI-shader', null, shaderIFragmentShader);

// 📝 Para aplicar este shader ao mundo, use:
// aplicarShaderAoSky('shaderI-shader', NUMERO_DO_MUNDO);
