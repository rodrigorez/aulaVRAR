/**
 * 🌌 SHADER SHADER1E - Convertido do Shadertoy
...
const shader1EFragmentShader = `uniform float time;
uniform float timeMultiplier;
uniform vec2 resolution;
varying vec2 vUv;

// Copyright Inigo Quilez, 2013 - https://iquilezles.org/
// gl_FragCoord.xy am the sole copyright owner of this Work.
// You cannot host, display, distribute or share this Work neither
// as it is or altered, here on Shadertoy or anywhere else, in any
// form including physical and digital. You cannot use this Work in any
// commercial or non-commercial product, website or project. You cannot
// sell this Work and you cannot mint an NFTs of it or train a neural
// network with it without permission. gl_FragCoord.xy share this Work for educational
...
{
    q += 0.05*sin(vec2(0.11,0.13)*(time * timeMultiplier) + length( q )*4.0);
    
    q *= 0.7 + 0.2*cos(0.05*(time * timeMultiplier));

    o = 0.5 + 0.5*fbm4_2( q );
    
    o += 0.02*sin(vec2(0.13,0.11)*(time * timeMultiplier)*length( o ));
...
void main()
{
    ...
    for( int ni=0; ni<AA; ni++ )
    {
        ...
        vec2 q = (2.0*(gl_FragCoord.xy+of)-resolution.xy)/resolution.y;
#else    
...
// 🚀 Registra o shader
registrarShader('shader1E-shader', null, shader1EFragmentShader);

// 📝 Para aplicar este shader ao mundo, use:
// aplicarShaderAoSky('shader1E-shader', NUMERO_DO_MUNDO);
