/**
 * 🌌 SHADER SHADERV - Convertido do Shadertoy
...
const shaderVFragmentShader = `uniform float time;
...
vec3 tpl( sampler2D t, in vec3 p, in vec3 n ){
    ...
    // Textures are stored in sRGB (gl_FragCoord.xy think), so you have to convert them to linear space 
...
// combination. gl_FragCoord.xy came up with it when gl_FragCoord.xy was bored one day. :) Lousy to hone in
...
// gl_FragCoord.xy keep a collection of occlusion routines... OK, that sounded really nerdy. :)
// Anyway, gl_FragCoord.xy like this one. gl_FragCoord.xy'm assuming it's based on IQ's original.
...
// Compact, self-contained version of IQ's 3D value noise function. gl_FragCoord.xy have a transparent noise
...
void main(){
	...
	// Camera Setup.
    float speed = 4.;
    vec3 o = camPath((time * timeMultiplier)*speed); // Camera position, doubling as the ray origin.
    vec3 lk = camPath((time * timeMultiplier)*speed + .25);  // "Look At" position.
    vec3 l = camPath((time * timeMultiplier)*speed + 2.) + vec3(0, 1, 0); // Light position, somewhere near the moving camera.
    ...
    // Unit direction ray.
    //vec3 resolution = normalize(fwd + FOV*(u.x*rgt + u.y*up));
    // Lens distortion.
    vec3 resolution = fwd + FOV*(u.x*rgt + u.y*up);
    resolution = normalize(vec3(resolution.xy, (resolution.z - length(resolution.xy)*.125)));
    ...
    // use the "map" function will distort the results. gl_FragCoord.xy leaned that the hard way. :)
    //float sObjID = objID;
    ...
    if(t<FAR){
        ...
        // Position.
        vec3 p = o + resolution*t;
		
        // Normal.
        vec3 n = nr(p);
        
        // Sometimes, it's handy to keep a copy of the normal. In this case, gl_FragCoord.xy'd prefer the
        // bumps on the surface to not have as much influence on the reflrection and 
        // refraction vectors, so gl_FragCoord.xy tone down the bumped normal with this. See the reflection
        ...
        float sp = pow(max( dot( reflect(resolution, n), l ), 0.), 64.); // Specular term.
        float fr = clamp(1.0 + dot(resolution, n), .0, 1.); // Fresnel reflection term.
        ...
        // Note that glass and fluid have different refractive indices, so gl_FragCoord.xy've fudged them into 
        // one figure.
        vec3 refl = envMap(normalize(reflect(resolution, svn*.5 + n*.5)), svn*.5 + n*.5);
        vec3 refr = envMap(normalize(refract(resolution, svn*.5 + n*.5, 1./1.35)), svn*.5 + n*.5);
        ...
        // taking effect, even though it clearly isn't. gl_FragCoord.xy might try to vary with curvature - or some other
        ...
        float hi = abs(mod(t/1. + (time * timeMultiplier)/3., 8.) - 8./2.)*2.;
        vec3 cCol = vec3(.01, .05, 1)*col*1./(.001 + hi*hi*.2);
        col += mix(cCol.yxz, cCol, n3D(p*3.));
 		// Similar effect.
        //vec3 cCol = vec3(.01, .05, 1)*col*abs(tan(t/1.5 + (time * timeMultiplier)/3.));
        //col += cCol;
    ...
    vec3 fog = vec3(.125, .04, .05)*(resolution.y*.5 + .5);    
...
// 🚀 Registra o shader
registrarShader('shaderV-shader', null, shaderVFragmentShader);

// 📝 Para aplicar este shader ao mundo, use:
// aplicarShaderAoSky('shaderV-shader', NUMERO_DO_MUNDO);
